	...
	public void Decode()
	{
		for(Expr E : this.ExprList)
		{
					...
					for(String s : writeVarsList)
					{
						if(!(this.FunctionCount == 1 && this.CurrentFunction.equals("main")))
						{
							for(SymbolTable ST : this.STS.SymbolTables)
							{
								...
								{
									int localIndex = 1;
									for(String local : ST.Locals)
									{
										if(s.equals(local))
										{
											if(SymbolLookup(s).equals("INT"))
											{ 
												this.NodeList.add(new IRNode("WRITEI", "$L" + String.valueOf(localIndex)));
											}
											else if(SymbolLookup(s).equals("FLOAT"))
											{ 
												this.NodeList.add(new IRNode("WRITEF", "$L" + String.valueOf(localIndex)));
											}
										}
										localIndex ++;
									}
			...
			else 
			{
				...
				{
					if(E.expr.contains("(") && E.expr.contains(")")) // function call
					{
						int paramCount = 0;
						String[] temp;
						String[] split;
						...
						for(String s : split)
						{
							for(SymbolTable ST : this.STS.SymbolTables)
							{
								if(ST.Scope.equals(this.CurrentFunction))
								{
									retType = ST.ReturnType;
									int localIndex = 1;
									for(String local : ST.Locals)
									{
										if(local.equals(s))
										{
											this.NodeList.add(new IRNode("PUSH", "$L" + String.valueOf(localIndex)));
											paramCount += 1;
										}
										localIndex ++;
									}
						...
						{
							this.NodeList.add(new IRNode("POP", "")); // Pop once for every function parameter
						}
						this.NodeList.add(new IRNode("POP", "$T" + String.valueOf(this.TempCounter))); // Pop the return value
						for(SymbolTable ST : this.STS.SymbolTables)
						{
							if(ST.Scope.equals(this.CurrentFunction))
							{
								...
								for(String local : ST.Locals)
								{
									if(E.id.equals(local))
									{
										if(SymbolLookup(E.id).equals("INT"))
										{ 
											this.NodeList.add(new IRNode("STOREI", "$T" + String.valueOf(this.TempCounter), "$L" + String.valueOf(localIndex)));
										}
										else if(SymbolLookup(E.id).equals("FLOAT"))
										{ 
											this.NodeList.add(new IRNode("STOREF", "$T" + String.valueOf(this.TempCounter), "$L" + String.valueOf(localIndex)));
										}
